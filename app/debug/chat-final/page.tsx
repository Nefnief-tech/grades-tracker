...
import { debugChatSystem, sendTeamMessage, getTeamMessages, testDirectMessage, fixAllTeamOwnership, debugMessageStorage } from '@/lib/teams-simple';
...
export default function ChatDebugPage() {
  ...
  const debugMessages = async () => {
    ...
    try {
      const userId = user.$id || (user as any).id || (user as any).userId;
      console.log('🔍 Debugging message storage for:', { teamId, userId });
      
      const result = await debugMessageStorage(teamId, userId);
      setDebugResult(result);
      
      toast({
        title: "Message Debug Completed",
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Enter team ID to test... (e.g., 685834d60030e99e1ef6)"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Button onClick={debugMessages} disabled={loading || !teamId} variant="outline">
              {loading ? 'Debugging...' : 'Debug Messages'}
            </Button>
          ...
          {debugResult && (
            <div className="p-4 bg-muted rounded-lg">
              ...
              <pre className="text-sm overflow-auto max-h-96">{JSON.stringify(debugResult, null, 2)}</pre>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <ol className="list-decimal list-inside space-y-2 text-sm">
            ...
            <li>Enter team ID: <code>685834d60030e99e1ef6</code> (from your console logs)</li>
            <li><strong>NEW:</strong> Click "Debug Messages" to see where messages are stored</li>
